...
export async function getFileContent(
  ...
  console.log("[GET_FILE_CONTENT] Repository:", `${site.githubOwner}/${site.githubRepo}`);
  ...
  try {
    ...
    console.log("[GET_FILE_CONTENT] Making API request...");
    ...
    try {
      ...
      console.log("[GET_FILE_CONTENT] Response type:", Array.isArray(response.data) ? 'directory' : response.data.type);
    } catch (error) {
    ...
    if (Array.isArray(response.data) || response.data.type !== "file") {
      console.error("[GET_FILE_CONTENT] Expected file but got:", Array.isArray(response.data) ? 'directory' : response.data.type);
      throw new Error(`Expected file at path ${path} but got ${Array.isArray(response.data) ? 'directory' : response.data.type}`);
    }

    const content = Buffer.from(response.data.content, "base64").toString("utf8");
    console.log("[GET_FILE_CONTENT] File content decoded, length:", content.length);
...
export async function validateGitHubPermissions(site: SiteConfig): Promise<{ valid: boolean; error?: string }> {
  ...
  try {
    ...
    // Test read permissions by getting repo info
    try {
    ...
    // Test write permissions by trying to get a reference (this requires repo scope)
    try {
      ...
      if (error instanceof RequestError && (error.status === 404 || error.status === 403)) {
        return {
          valid: false,
          error: `Insufficient permissions for repository ${owner}/${repo}. The GitHub token needs 'repo' scope to write to the repository. Current OAuth scopes: ${error.response?.headers?.['x-oauth-scopes'] || 'None'}`
        };
      }
      throw error;
    }
    ...
    return {
      valid: false,
      error: `Failed to validate GitHub permissions: ${error instanceof Error ? error.message : 'Unknown error'}`
    };
  }
}
...
export async function publishFiles(
  ...
  console.log("[PUBLISH_FILES] Repository:", `${site.githubOwner}/${site.githubRepo}`);
  ...
  try {
    ...
    files.forEach((file, index) => {
      console.log(`[PUBLISH_FILES] File ${index + 1}:`, file.path, "Content length:", file.content?.length || 0);
    });

    console.log("[PUBLISH_FILES] Getting reference for branch:", `heads/${site.githubBranch}`);
    const { data: ref } = await octokit.git.getRef({
      owner,
      repo,
      ref: `heads/${site.githubBranch}`,
    });
    console.log("[PUBLISH_FILES] Reference retrieved successfully, SHA:", ref.object.sha);
    ...
    console.log("[PUBLISH_FILES] Latest commit retrieved successfully, tree SHA:", latestCommit.tree.sha);

    console.log("[PUBLISH_FILES] Creating tree with base_tree:", latestCommit.tree.sha);
    ...
    console.log("[PUBLISH_FILES] Commit created successfully, SHA:", commit.sha);

    console.log("[PUBLISH_FILES] Updating reference:", `heads/${site.githubBranch}`);
    ...
    console.log("[PUBLISH_FILES] Publish completed successfully, URL:", pagesUrl);
